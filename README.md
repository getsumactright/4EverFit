...
> **Note — corrected 2026-08-10.** This table previously listed flat hex values that did not match
> `styles.css`, the file both `.dc.html` designs actually link and render against (no local overrides in
> either file). The values below are read directly from `styles.css` and are what the designs actually
> show. If you ever see this table and the stylesheet disagree again, trust the stylesheet — treat that as
> a bug in this doc, not in the CSS.
...
| `--color-divider` | `color-mix(in srgb, #201e1d 40%, transparent)` — an **alpha-composited** ink tint, not a flat hex. Over `--color-bg` it renders roughly `#9f9d9d`, noticeably darker/stronger than the flat gray a static token would suggest. Because it carries alpha rather than a solid color, its rendered value depends on whatever sits behind it — fine everywhere it sits on `--color-bg`, but worth a visual check at the dark-band seam (section 5) and anywhere it might be ported into a system that only accepts flat/static color tokens (Figma variables, React Native, etc.), where it should be flattened to its computed hex per context. | Every 2px rule |
| `--color-surface` | `#eae9e9` — **darker** than `--color-bg` (#f3f2f2), which reads as recessed rather than raised. Confirm this is intentional before porting; it is not used directly in either page design (mostly backs `.input` fields). | Raised surface |
...
| --- | --- | --- |
| `--color-neutral-100` | `#f8f4f4` | Assessment card fill; dark-band heading text |
| `--color-neutral-200` | `#eae7e7` | Page ground behind the mobile device frame |
| `--color-neutral-300` | `#d7d3d3` | Inactive progress segment (mobile) |
| `--color-neutral-400` | `#bab6b6` | Unchecked step marker border (desktop) |
| `--color-neutral-500` | `#9b9797` | Unchecked radio border |
| `--color-neutral-600` | `#7d7979` | Muted meta text ("Question 1 of 4") |
| `--color-neutral-700` | `#605d5d` | Label / caption text; dark-band rules |
| `--color-neutral-800` | `#444141` | Body paragraph text |
| `--color-neutral-900` | `#2d2b2b` | Dark band background ("How a session runs") |
| `--color-neutral-300` (on dark) | `#d7d3d3` | Body copy inside the dark band |
| `--color-accent-100` | `#fff2ef` | Assessment option hover fill |
| `--color-accent-400` | `#ff9783` | Accent on dark ground (kickers, step numerals in dark band) |
| `--color-accent-700` | `#ae1800` | **Accent at paragraph text size** — kickers, result stats, error text |

`--color-accent-2` (`#e15b47`) and its ramp are also defined in `styles.css` but are unused in both
`.dc.html` designs (confirmed by search) — they're the machine-derived stand-in the system keeps so a
two-accent token shape resolves even though this is a mono palette. Leave them out of anything you port
unless a second accent gets deliberately chosen later.
